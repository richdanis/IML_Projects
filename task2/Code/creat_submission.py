...
pred_st1 = 'Data/pred_st12_Sven.csv'
...
df_sub = pd.merge(df_st1,df_st3,on='pid')
#df = pd.merge(df_sub,df_sample_3,on='pid')

#assert df_sub.shape == df_sample.shape
#assert df_sub.columns == df_sub.columns

df_sub.to_csv('Data/Submission_Sven.csv', index=False, float_format='%.3f')
